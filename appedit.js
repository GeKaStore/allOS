...
// âœ… Tambahan proteksi global
const fs = require('fs');
const ADMIN_ID = process.env.ADMIN_ID || '1210833546';
...
// ðŸ§  Tambahkan di awal handler message
bot.on('message', async (msg) => {
  const chatId = msg.chat.id;
  global.lastUserId = chatId;

  const blacklist = getBlacklist();
  if (blacklist.includes(chatId)) {
    return bot.sendMessage(chatId, 'ðŸš« Kamu telah diblokir karena menyebabkan crash.');
  }
...
const GROUP_CHAT_ID = vars.GROUP_CHAT_ID;
...
const db = new sqlite3.Database('./sellvpn.db', (err) => {
    if (err) {
        ...
        db.serialize(() => {
            ...
            `, (err) => {
                if (err) logger.error('âŒ Gagal membuat tabel bonus_config:', err.message);
                else logger.info('âœ… Tabel bonus_config siap');
            });
            ...
            db.run(`
                ...
                VALUES (1, 0, 0, 0)
            `, (err) => {
                if (err) logger.error('âŒ Gagal insert default bonus_config:', err.message);
                else logger.info('âœ… Default bonus_config dijamin ada');
            });
            ...
            `, (err) => {
                if (err) logger.error('âŒ Gagal membuat tabel bonus_log:', err.message);
                else logger.info('âœ… Tabel bonus_log siap');
            });
            ...
            `, (err) => {
                if (err) {
                    logger.error('âŒ Gagal membuat tabel pending_deposits:', err.message);
                } else {
            ...
            `, (err) => {
                if (err) {
                    logger.error('âŒ Gagal membuat tabel log_penjualan:', err.message);
                } else {
...
`, (err) => {
  if (err) logger.error('âŒ Gagal membuat tabel topup_log:', err.message);
  else logger.info('âœ… Tabel topup_log siap');
});
...
bot.command(['start', 'menu'], async (ctx) => {
  logger.info('ðŸ“¥ Perintah /start atau /menu diterima');
  ...
  try {
    ...
    logger.info(`ðŸ§¹ Pesan command user ${userId} berhasil dihapus`);
  } catch (e) {
    console.warn(`âš ï¸ Tidak bisa hapus pesan command user ${userId}:`, e.message);
  }
  ...
  db.get('SELECT * FROM users WHERE user_id = ?', [userId], (err, row) => {
    ...
    if (!row) {
      db.run('INSERT INTO users (user_id) VALUES (?)', [userId], (err) => {
        if (err) {
          logger.error('âŒ Gagal menyimpan user_id:', err.message);
        } else {
          logger.info(`âœ… User ID ${userId} berhasil disimpan`);
        }
      });
    } else {
      logger.info(`â„¹ï¸ User ID ${userId} sudah ada`);
    }
  });
  ...
  if (lastMenus[userId]) {
    try {
      ...
      logger.info(`ðŸ§¹ Menu lama milik ${userId} dihapus`);
    } catch (e) {
      console.warn(`âš ï¸ Gagal hapus menu lama user ${userId}:`, e.message);
    }
  }
  ...
  if (sent?.message_id) {
    ...
    logger.info(`âœ… Menu baru dikirim ke ${userId} dengan message_id ${sent.message_id}`);
  }
});
...
bot.command('admin', async (ctx) => {
  ...
  if (!adminIds.includes(ctx.from.id)) {
    await ctx.reply('âŒ Anda tidak memiliki izin untuk mengakses menu admin.');
    return;
  }
...
// Contoh di fungsi sendPaymentSuccessNotification ATAU setelah updateUserBalance
...
async function sendMainMenu(ctx) {
  const keyboard = [
  [{ text: 'ðŸŒŸ Trial Akun', callback_data: 'service_trial' }],
  [{ text: 'âœï¸ Buat Akun', callback_data: 'service_create' }, { text: 'â™»ï¸ Renew Akun', callback_data: 'service_renew' }],
  [{ text: 'ðŸ›’ Sewa Script', callback_data: 'service_sewascript' }],   // <--- Tambahkan baris ini
  [{ text: 'ðŸ’³ TopUp Saldo', callback_data: 'menu_topup' }]
];
...
â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”
        ðŸ·ï¸ *â‰¡ BOT PANEL VPN â‰¡* ðŸ·ï¸
â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”
Selamat datang di *${NAMA_STORE}* âœ¨
Bot serba otomatis untuk pembelian  
layanan VPN dengan mudah, cepat,  
dan aman. Nikmati kenyamanan  
transaksi hanya lewat bot ðŸ†
ðŸ¤– *Â» Bot Aktif:* \`${uptimeFormatted}\`
â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”
ðŸ’³ *Â» Saldo:* \`Rp.${saldo}\`
â‡ï¸ *Â» Trial Bisa 2x Sehari Juragan*
ðŸš€ *Â» Support Wildcard*
â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”
ðŸ•’ *Â» Jam:* \`${timeNow}\`
ðŸ“… *Â» Hari:* \`${currentDay}, ${currentDate}\`
ðŸ‘‘ *Â» Username:* \`${username}\`
ðŸ†” *Â» ID Pengguna:* \`${userId}\`
ðŸŒ *Â» Server Aktif:* \`${jumlahServer}\`
ðŸ‘¥ *Â» Total User:* \`${jumlahPengguna}\`
â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”
â™‚ï¸ *Â» Admin:* [@kytxz](https://t.me/kytxz)`;
...
bot.command('broadcast', async (ctx) => {
  ...
  if (!reply && !inputText) {
    return ctx.reply(
      'ðŸ“Œ *Cara menggunakan perintah broadcast:*\n\n' +
  ...
  db.all("SELECT user_id FROM users", [], async (err, rows) => {
    if (err) {
      ...
      return ctx.reply('âš ï¸ Gagal mengambil daftar pengguna.');
    }
    ...
    for (const row of rows) {
      try {
        ...
        failed++;
        console.error(`âŒ Gagal kirim ke ${row.user_id}:`, error.message);
      }
...
bot.action(/^batal_topup_(.+)$/, async (ctx) => {
  ...
  try {
    ...
    await ctx.reply('âŒ Topup QRIS Orkut telah dibatalkan. Silakan topup ulang jika ingin mencoba lagi.', {
      parse_mode: 'Markdown',
      reply_markup: {
        inline_keyboard: [
          [{ text: 'ðŸ”™ Kembali ke Menu Top-up', callback_data: 'menu_topup' }]
        ]
      }
    });
    // =============================================
...
bot.action('statistik_penjualan', async (ctx) => {
  ...
  `, [], (err, rows) => {
    if (err || rows.length === 0) {
      return ctx.reply('âš ï¸ Belum ada data penjualan.');
    }
    ...
    db.get(`SELECT SUM(harga) AS total FROM log_penjualan WHERE waktu_transaksi >= ?`, [startOfMonth], (err4, monthRow) => {
      ...
      const message =
        `ðŸ“Š *Statistik Penjualan per Tipe Akun:*\n\n${hasil}\n\n` +
        `ðŸ§¾ *Total Semua Akun Terjual:* ${totalAkun}\n` +
        `ðŸ’° *Total Uang Masuk:* ${formatRupiah(totalUang)}\n\n` +
        `ðŸ“… *Hari Ini:* ${formatRupiah(totalToday)}\n` +
        `ðŸ“ˆ *7 Hari Terakhir:* ${formatRupiah(total7Days)}\n` +
        `ðŸ—“ï¸ *Minggu Ini:* ${formatRupiah(totalWeek)}\n` +
        `ðŸ“† *Bulan Ini:* ${formatRupiah(totalMonth)}`;
...
bot.action('menu_topup', async (ctx) => {
  ...
  if (config.topup_saldo)
    keyboard.push([{ text: "ðŸ’¸ Topup QRIS Orkut", callback_data: "topup_saldo" }]);
  if (config.topup_saweria)
    keyboard.push([{ text: "ðŸ’¸ Topup QRIS Saweria", callback_data: "topup_saweria" }]);
  keyboard.push([{ text: "ðŸ”™ Kembali", callback_data: "send_main_menu" }]);
...
â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”
        ðŸ·ï¸ *â‰¡ BOT PANEL VPN â‰¡* ðŸ·ï¸
â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”
ðŸ’¸ *Â» Pilih Menu Topup Dibawah Ini:*`;
...
// ========== HANDLE INPUT NOMINAL (KEDUA MODE) ==========



// ========== FUNGSI PROSES TOPUP QRIS SAWERIA ==========
async function processDepositSaweria(ctx, amount) {
  try {
    ...
    if (!SAWERIA_USERNAME || !SAWERIA_EMAIL) {
      return ctx.reply('âŒ *Konfigurasi Saweria belum lengkap.*', { parse_mode: 'Markdown' });
    }
    ...
    if (!result.success || !result.data?.qrImageUrl || !result.data?.transactionId) {
      return ctx.reply('âŒ *Gagal membuat QR Saweria.*', { parse_mode: 'Markdown' });
    }

    const { qrImageUrl, transactionId, checkPaymentUrl, timestamp } = result.data;

    // Simpan transaksi ke global.qrisTransaksi (ini yang sudah ada)
    if (!global.qrisTransaksi) global.qrisTransaksi = {};
    global.qrisTransaksi[transactionId] = {
      user_id: ctx.from.id,
      amount: amountInt,
      created_at: Date.now()
    };

    // --- TAMBAHKAN KODE INI UNTUK MENYIMPAN KE global.pendingDepositsSaweria ---
    if (!global.pendingDepositsSaweria) global.pendingDepositsSaweria = {};
    global.pendingDepositsSaweria[transactionId] = {
      userId: ctx.from.id, // Gunakan userId agar konsisten dengan objek lain di pendingDepositsSaweria
      amount: amountInt,
      created_at: Date.now(),
      checked: false // Tambahkan properti 'checked' untuk menandai apakah sudah diperiksa
    };
    // -------------------------------------------------------------------------

    // Kirim QR ke user
    await ctx.replyWithPhoto(qrImageUrl, {
      caption: `ðŸŒŸ *Informasi Deposit Anda* ðŸŒŸ
ðŸ§¾ *Kode Transaksi:* \`${transactionId}\`
ðŸ’¼ *Jumlah:* Rp${amountInt.toLocaleString('id-ID')}
ðŸ“… *Waktu:* ${timestamp}
ðŸ”— [Cek Status Pembayaran](${checkPaymentUrl})

ðŸ–¼ *Silakan scan QR berikut untuk membayar melalui QRIS.*
â± *Expired:* 5 menit dari sekarang`,
      parse_mode: 'Markdown',
    });

  } catch (err) {
    logger.error('âŒ Gagal proses QRIS Saweria:', err.message);
    await ctx.reply('âŒ *Gagal membuat QRIS Saweria.* Silakan coba lagi nanti.', { parse_mode: 'Markdown' });
  }
}
...
async function processDepositSaweria(ctx, amount) {
  try {
    ...
    global.pendingDepositsSaweria[transactionId] = {
      userId: ctx.from.id, // Gunakan userId agar konsisten dengan objek lain di pendingDepositsSaweria
      username: ctx.from.username || `user_${ctx.from.id}`, // <<< TAMBAHKAN INI UNTUK USERNAME
      amount: amountInt,
      created_at: Date.now(),
      checked: false // Tambahkan properti 'checked' untuk menandai apakah sudah diperiksa
    };
    ...
    // Kirim QR ke user
    const qrMessage = await ctx.replyWithPhoto(qrImageUrl, { // << Simpan objek pesan untuk mendapatkan message_id
      caption: `ðŸŒŸ *Informasi Deposit Anda* ðŸŒŸ
ðŸ§¾ *Kode Transaksi:* \`${transactionId}\`
ðŸ’¼ *Jumlah:* Rp${amountInt.toLocaleString('id-ID')}
ðŸ“… *Waktu:* ${timestamp}
...
â± *Expired:* 5 menit dari sekarang`,
    ...
    // Simpan message_id dari pesan QR agar bisa dihapus atau diedit nanti
    global.pendingDepositsSaweria[transactionId].qrMessageId = qrMessage.message_id; // <<< TAMBAHKAN INI
...
// ========== CEK STATUS PEMBAYARAN SAWERIA QRIS (otomatis 10 detik) ==========
setInterval(async () => {
  if (!global.pendingDepositsSaweria || Object.keys(global.pendingDepositsSaweria).length === 0) return;

  const transactionsToCheck = Object.entries(global.pendingDepositsSaweria);

  for (const [idtrx, dep] of transactionsToCheck) {
    ...
    try {
      const cek = await axios.get(`https://saweria.autsc.my.id/check-payment?idtransaksi=${idtrx}`);
      const saweriaData = cek.data.data;

      logger.info(`Respons Saweria check-payment untuk ${idtrx}: ${JSON.stringify(cek.data)}`);

      if (cek.data && cek.data.success && saweriaData && saweriaData.isPaid) {
        dep.checked = true;
        ...
        // Bonus (opsional)
        prosesBonusTopUp(dep.userId, dep.username, dep.amount);
        logTopup(dep.userId, dep.username, dep.amount, 'Saweria');

        // Tentukan bonus dan originalAmount jika pakai sistem bonus
        const bonus = dep.bonus || 0;
        const bonus_percent = dep.bonus_percent || 0;
        const originalAmount = dep.amount;
        ...
        const success = await sendPaymentSuccessNotification(dep.userId, depositData);
...
async function handleServiceAction(ctx, action) {
  ...
  if (action === 'trial') {
    keyboard = [
      [{ text: 'ðŸŒŸ Trial SSH', callback_data: 'trial_ssh' }],
      [{ text: 'ðŸŒŸ Trial Vmess', callback_data: 'trial_vmess' }, { text: 'ðŸŒŸ Trial Vless', callback_data: 'trial_vless' }],
      [{ text: 'ðŸŒŸ Trial Trojan', callback_data: 'trial_trojan' }, { text: 'ðŸŒŸ Trial Shadowsocks', callback_data: 'trial_shadowsocks' }],
      [{ text: 'ðŸ”™ Kembali', callback_data: 'send_main_menu' }]
    ];
  } else if (action === 'create') {
    keyboard = [
      [{ text: 'â™‚ï¸ SSH', callback_data: 'create_ssh' }],
      [{ text: 'â™‚ï¸ Vmess', callback_data: 'create_vmess' }, { text: 'â™‚ï¸ Vless', callback_data: 'create_vless' }],
      [{ text: 'â™‚ï¸ Trojan', callback_data: 'create_trojan' }, { text: 'â™‚ï¸ Shadowsocks', callback_data: 'create_shadowsocks' }],
      [{ text: 'ðŸ”™ Kembali', callback_data: 'send_main_menu' }]
    ];
  } else if (action === 'sewascript') {
    keyboard = [
      [{ text: 'ðŸ¥‡ Daftar IP', callback_data: 'sewascript_daftar' }, { text: 'ðŸ¥ˆ Renew IP', callback_data: 'sewascript_perpanjang' }],
      [{ text: 'ðŸ”™ Kembali', callback_data: 'send_main_menu' }]
    ];
  } else if (action === 'renew') {
    keyboard = [
      [{ text: 'ðŸŒ€ Renew SSH', callback_data: 'renew_ssh' }],
      [{ text: 'ðŸŒ€ Renew Vmess', callback_data: 'renew_vmess' }, { text: 'ðŸŒ€ Renew Vless', callback_data: 'renew_vless' }],
      [{ text: 'ðŸŒ€ Renew Trojan', callback_data: 'renew_trojan' }, { text: 'ðŸŒ€ Renew Shadowsocks', callback_data: 'renew_shadowsocks' }],
      [{ text: 'ðŸ”™ Kembali', callback_data: 'send_main_menu' }]
    ];
  }
  try {
...
async function sendAdminMenu(ctx) {
    ...
    const adminKeyboard = [
        [{ text: 'âœï¸ Tambah Server', callback_data: 'addserver' }, { text: 'âŒ Hapus Server', callback_data: 'deleteserver' }],
        [{ text: 'ðŸ’² Edit Harga', callback_data: 'editserver_harga' }, { text: 'ðŸ“ Edit Nama', callback_data: 'nama_server_edit' }],
        [{ text: 'ðŸŒ Edit Domain', callback_data: 'editserver_domain' }, { text: 'ðŸ”‘ Edit Auth', callback_data: 'editserver_auth' }],
        [{ text: 'ðŸ“Š Edit Quota', callback_data: 'editserver_quota' }, { text: 'ðŸ“¶ Edit Limit IP', callback_data: 'editserver_limit_ip' }],
        [{ text: 'ðŸ”¢ Edit Batas Create', callback_data: 'editserver_batas_create_akun' }, { text: 'ðŸ”¢ Edit Total Create', callback_data: 'editserver_total_create_akun' }],
        [{ text: 'ðŸ’µ Tambah Saldo', callback_data: 'addsaldo_user' }, { text: 'ðŸ“‹ List Server', callback_data: 'listserver' }],
        [{ text: 'â™»ï¸ Reset Server', callback_data: 'resetdb' }, { text: 'â„¹ï¸ Detail Server', callback_data: 'detailserver' }],
        [{ text: 'ðŸŽ Set Bonus TopUp', callback_data: 'bonus_topup_setting' }, { text: 'ðŸ“œ Log Bonus TopUp', callback_data: 'log_bonus_topup' }],
        [{ text: `${config.topup_saldo ? 'âœ…' : 'âŒ'} Topup QRIS Orkut`, callback_data: 'toggle_topup_saldo' },{ text: `${config.topup_saweria ? 'âœ…' : 'âŒ'} Topup QRIS Saweria`, callback_data: 'toggle_topup_saweria' }],
        [{ text: 'ðŸ“ˆ Hasil Penjualan', callback_data: 'statistik_penjualan' },{ text: 'ðŸ“‘ Log Topup', callback_data: 'log_topup' }],
        [{ text: 'ðŸ”™ Kembali', callback_data: 'send_main_menu' }]
    ];

    const messageText = `
â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”
        ðŸ·ï¸ *â‰¡ MENU ADMIN VPN â‰¡* ðŸ·ï¸
â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”
ðŸ’¸ *Â» Pilih Menu Admin Dibawah Ini:*`;
...
bot.action('sewascript_daftar', async (ctx) => {
    ...
    await ctx.reply('ðŸ“… Pilih Durasi Sewa Script:', {
        reply_markup: {
            inline_keyboard: [
                [
                ...
                [{ text: 'ðŸ”™ Kembali', callback_data: 'service_sewascript' }]
            ]
        }
    });
});
bot.action('sewascript_perpanjang', async (ctx) => {
    ...
    await ctx.reply('ðŸ“… Pilih Durasi Perpanjangan Script:', {
        reply_markup: {
            inline_keyboard: [
                [
...
bot.action(/^daftar_(\d+)bln$/, async (ctx) => {
  ...
  await ctx.reply('â™‚ï¸ *Masukkan username:*', { parse_mode: 'Markdown' });
});
...
bot.action(/^perpanjang_(\d+)bln$/, async (ctx) => {
  ...
  await ctx.reply('ðŸŒ€ *Masukkan IP yang ingin diperpanjang:*', { parse_mode: 'Markdown' });
});
...
try {
  ...
  if (!ctx || !ctx.match) {
    return ctx.reply('âŒ *GAGAL!* Terjadi kesalahan saat memproses permintaan Anda. Silahkan coba lagi nanti.', { parse_mode: 'Markdown' });
  }
...
bot.action('trial_ssh', async (ctx) => {
  if (!ctx || !ctx.match) { return ctx.reply('âŒ *GAGAL!* Terjadi kesalahan saat memproses permintaan Anda. Silahkan coba lagi nanti.', { parse_mode: 'Markdown' }); }
  ...
  try {
    ...
    if (lastMenus[userId]) {
      try {
        ...
        console.warn(`âš ï¸ Gagal hapus menu lama dari ${userId}:`, e.message);
      }
    }
    ...
    logger.error('âŒ Gagal handle send_main_menu:', error.message);
    await ctx.reply('âŒ *Gagal memproses menu utama.*', { parse_mode: 'Markdown' });
  }
});
  ...
  try {
    ...
    db.all('SELECT * FROM Server', [], (err, servers) => {
      ...
      const serverList = currentServers.map(server => {
        ...
        return `ðŸŒ *${server.nama_server}*\n` +
               `ðŸ’° Harga per hari: Rp${server.harga}\n` +
               `ðŸ“… Harga per 30 hari: Rp${hargaPer30Hari}\n` +
               `ðŸ“Š Quota: ${server.quota}GB\n` +
               `ðŸ”¢ Limit IP: ${server.iplimit} IP\n` +
               (isFull ? `âš ï¸ *Server Penuh*` : `ðŸ‘¥ Total Create Akun: ${server.total_create_akun}/${server.batas_create_akun}`);
      }).join('\n\n');

      if (ctx.updateType === 'callback_query') {
        ctx.editMessageText(`ðŸ“‹ *List Server (Halaman ${currentPage + 1} dari ${totalPages}):*\n\n${serverList}`, {
          reply_markup: {
            inline_keyboard: keyboard
          },
          parse_mode: 'Markdown'
        ...
        ctx.reply(`ðŸ“‹ *List Server (Halaman ${currentPage + 1} dari ${totalPages}):*\n\n${serverList}`, {
          reply_markup: {
            inline_keyboard: keyboard
          },
          parse_mode: 'Markdown'
    ...
    await ctx.reply(`âŒ *GAGAL!* Terjadi kesalahan saat memproses permintaan Anda. Silahkan coba lagi nanti.`, { parse_mode: 'Markdown' });
  }
}
...
bot.action(/^(create|renew|trial)_username_(vmess|vless|trojan|shadowsocks|ssh)_(.+)$/, async (ctx) => {
  ...
  if (!action || !type || !serverId) {
    return ctx.reply('âŒ *Perintah tidak dikenali.*', { parse_mode: 'Markdown' });
  }
  ...
  db.get('SELECT batas_create_akun, total_create_akun FROM Server WHERE id = ?', [serverId], (err, server) => {
    if (err) {
      logger.error('âŒ Error fetching server details:', err.message);
    ...
    if (total_create_akun >= batas_create_akun) {
      return ctx.reply('âŒ *Server penuh. Trial tidak dapat dibuat di server ini.*', { parse_mode: 'Markdown' });
    }

    db.get('SELECT count FROM TrialLog WHERE user_id = ? AND date = ?', [userId, today], async (err, row) => {
      if (err) {
        logger.error('âŒ Error saat cek log trial:', err);
        return ctx.reply('âŒ *Terjadi kesalahan saat memproses trial. Silahkan coba lagi nanti.*', { parse_mode: 'Markdown' });
      }

      const trialCount = row?.count || 0;

      if (trialCount >= 2) {
        return ctx.reply('âš ï¸ *Kamu sudah trial hari ini, Gass Order* ðŸ˜–', { parse_mode: 'Markdown' });
      }
    ...
    db.get('SELECT batas_create_akun, total_create_akun FROM Server WHERE id = ?', [serverId], async (err, server) => {
      ...
      if (total_create_akun >= batas_create_akun) {
        return ctx.reply('âŒ *Server penuh. Tidak dapat membuat akun baru di server ini.*', { parse_mode: 'Markdown' });
      }

      await ctx.reply('ðŸ‘¤ *Masukkan username:*', { parse_mode: 'Markdown' });
    });
  }
});
  ...
  try {
    ...
    switch (type) {
      ...
      case 'ssh':
        ...
        break;
      default:
        msg = 'âŒ *Tipe layanan tidak dikenali.*';
    }
    ...
    logger.error(`âŒ Error trial ${type}:`, error);
    await ctx.reply('âŒ *Gagal membuat akun trial. Silahkan coba lagi nanti.*', { parse_mode: 'Markdown' });
  } finally {
...
async function showLoading(ctx, durasi = 20000) {
  const waitMsg = await ctx.reply("â³ Loading");
...
bot.on('text', async (ctx) => {
  ...
  // ðŸ” Debug log input teks user
  console.log(`ðŸ“© Input dari ${userId}: ${ctx.message.text}`);
  logger.info(`ðŸ“© Input teks dari ${userId}: ${ctx.message.text}`);

  // âœ… Tangani input nominal topup Saweria dulu
  if (global.depositState && global.depositState[userId]?.action === 'request_amount_saweria') {
    ...
    if (isNaN(nominal) || nominal < 1000) {
      return ctx.reply('âŒ *Nominal tidak valid. Minimal Rp1000.*', { parse_mode: 'Markdown' });
    }

    delete global.depositState[userId]; // hapus state
    await ctx.reply(`ðŸ”„ Memproses QRIS Saweria untuk Rp${nominal}...`);
    await processDepositSaweria(ctx, nominal);
    return;
  }

  // ðŸ‘¤ Input username untuk sewa script
  if (state && state.step === 'sewascript_create_input') {
    const username = ctx.message.text.trim();

    if (!/^[a-zA-Z0-9]{3,20}$/.test(username)) {
      return ctx.reply('âŒ *Username tidak valid. Harus 3-20 karakter alfanumerik.*', { parse_mode: 'Markdown' });
    }
    ...
    await ctx.reply('ðŸ·ï¸ *Masukkan IP Address:*', { parse_mode: 'Markdown' });
    return;
  }

  // ðŸŒ Input IP Address untuk daftar script
  if (state && state.step === 'sewascript_create_input_ip') {
    const ip = ctx.message.text.trim();
    const { username, bulan } = state;

    if (!/^\d{1,3}(\.\d{1,3}){3}$/.test(ip)) {
      return ctx.reply('âŒ *Format IP tidak valid.* Masukkan IP seperti 123.45.67.89', { parse_mode: 'Markdown' });
    }
    ...
    db.get('SELECT saldo FROM users WHERE user_id = ?', [userId], async (err, user) => {
      if (err || !user) {
        return ctx.reply('âŒ Terjadi kesalahan mengambil saldo pengguna.', { parse_mode: 'Markdown' });
      }

      if (user.saldo < priceharga) {
        return ctx.reply('âŒ *Saldo Anda tidak cukup.*', { parse_mode: 'Markdown' });
      }
      ...
      exec(cmd, async (error, stdout, stderr) => {
        ...
        const output = error || /gagal|error/i.test(stdout)
          ? `âŒ Gagal daftar script:\n\n${stdout || stderr}`
          : `âœ… Pendaftaran IP Berhasil:\n${stdout}`;
    ...
    db.get('SELECT saldo FROM users WHERE user_id = ?', [userId], async (err, user) => {
        ...
        if (user.saldo < priceharga) {
            return ctx.reply('âŒ *Saldo Anda tidak cukup untuk memperpanjang.*', { parse_mode: 'Markdown' });
        }
        ...
        exec(cmd, async (error, stdout, stderr) => {
            ...
            const output = error || /gagal|error/i.test(stdout)
            ? `âŒ Gagal memperpanjang script:\n\n${stdout || stderr}`
            : `âœ… Perpanjangan IP Berhasil:\n${stdout}`;
    ...
    if (state && state.step === 'atur_bonus_input') {
        ...
        if (!status || isNaN(min) || isNaN(persen)) {
            return ctx.reply('âš ï¸ Format salah. Gunakan: `on|off <minimal_topup> <persen_bonus>`\nContoh: `on 10000 25`', { parse_mode: 'Markdown' });
        }
        ...
        db.run('UPDATE bonus_config SET enabled = ?, min_topup = ?, bonus_percent = ? WHERE id = 1',
            ...
            (err) => {
                if (err) {
                    logger.error('âŒ Gagal update bonus config:', err.message);
                    return ctx.reply('âŒ Gagal menyimpan pengaturan bonus.');
                }
                ctx.reply(`âœ… Bonus Top Up *${enabled ? 'Aktif' : 'Nonaktif'}*
ðŸ“Œ Minimal Top Up: Rp${min}
ðŸŽ Bonus: ${persen}%`, {
                    parse_mode: 'Markdown'
                });
                delete userState[userId];
    ...
    if (state && state.step.startsWith('username_')) {
        ...
        if (state.username.length < 3 || state.username.length > 20) {
            return ctx.reply('âŒ *Username harus terdiri dari 3 hingga 20 karakter.*', { parse_mode: 'Markdown' });
        }
        if (/[^a-zA-Z0-9]/.test(state.username)) {
            return ctx.reply('âŒ *Username tidak boleh mengandung karakter khusus atau spasi.*', { parse_mode: 'Markdown' });
        }
    ...
    if (state && state.step.startsWith('password_')) {
        ...
        if (state.password.length < 1) {
            return ctx.reply('âŒ *Password harus terdiri dari minimal 1 karakter.*', { parse_mode: 'Markdown' });
        }
            ...
            db.get('SELECT saldo FROM users WHERE user_id = ?', [userId], async (err, user) => {
                ...
                if (state.action === 'create') {
                    switch (state.type) {
                        ...
                        await recordAccountTransaction(ctx.from.id, 'shadowshock');
                ...
                if (!successAction) {
                    delete userState[userId];
                    return ctx.reply('âŒ *Pembuatan/Renew akun gagal. Saldo tidak dipotong.*', { parse_mode: 'Markdown' });
                }
                ...
                db.run('UPDATE Server SET total_create_akun = total_create_akun + 1 WHERE id = ?', [state.serverId], (err) => {
                    ...
                    db.run(`INSERT INTO log_penjualan (
                        ...
                        if (err) {
                            logger.warn('âš ï¸ Gagal mencatat log penjualan:', err.message);
                        } else {
                            logger.info(`âœ… Log penjualan dicatat: ${ctx.from.id} - ${state.type} - ${state.action} - Rp${totalHarga}`);
                        }
                    });
    ...
    if (state.step === 'addserver') {
        ...
        if (!domain) {
            return ctx.reply('âš ï¸ *Domain tidak boleh kosong.* Silahkan masukkan domain server yang valid.', { parse_mode: 'Markdown' });
        }
        userState[userId].step = 'addserver_auth';
        userState[userId].domain = domain;
        await ctx.reply('ðŸ”‘ *Silahkan masukkan auth server:*', { parse_mode: 'Markdown' });
        return;
    } else if (state.step === 'addserver_auth') {
        const auth = ctx.message.text.trim();
        if (!auth) {
            return ctx.reply('âš ï¸ *Auth tidak boleh kosong.* Silahkan masukkan auth server yang valid.', { parse_mode: 'Markdown' });
        }
        userState[userId].step = 'addserver_nama_server';
        userState[userId].auth = auth;
        await ctx.reply('ðŸ·ï¸ *Silahkan masukkan nama server:*', { parse_mode: 'Markdown' });
        return;
    } else if (state.step === 'addserver_nama_server') {
        const nama_server = ctx.message.text.trim();
        if (!nama_server) {
            return ctx.reply('âš ï¸ *Nama server tidak boleh kosong.* Silahkan masukkan nama server yang valid.', { parse_mode: 'Markdown' });
        }
        userState[userId].step = 'addserver_quota';
        userState[userId].nama_server = nama_server;
        await ctx.reply('ðŸ“Š *Silahkan masukkan quota server:*', { parse_mode: 'Markdown' });
        return;
    } else if (state.step === 'addserver_quota') {
        const quota = parseInt(ctx.message.text.trim(), 10);
        if (isNaN(quota)) {
            return ctx.reply('âš ï¸ *Quota tidak valid.* Silahkan masukkan quota server yang valid.', { parse_mode: 'Markdown' });
        }
        userState[userId].step = 'addserver_iplimit';
        userState[userId].quota = quota;
        await ctx.reply('ðŸ”¢ *Silahkan masukkan limit IP server:*', { parse_mode: 'Markdown' });
        return;
    } else if (state.step === 'addserver_iplimit') {
        const iplimit = parseInt(ctx.message.text.trim(), 10);
        if (isNaN(iplimit)) {
            return ctx.reply('âš ï¸ *Limit IP tidak valid.* Silahkan masukkan limit IP server yang valid.', { parse_mode: 'Markdown' });
        }
        userState[userId].step = 'addserver_batas_create_akun';
        userState[userId].iplimit = iplimit;
        await ctx.reply('ðŸ”¢ *Silahkan masukkan batas create akun server:*', { parse_mode: 'Markdown' });
        return;
        ...
        if (isNaN(batas_create_akun)) {
            return ctx.reply('âš ï¸ *Batas create akun tidak valid.* Silahkan masukkan batas create akun server yang valid.', { parse_mode: 'Markdown' });
        }
        userState[userId].step = 'addserver_harga';
        userState[userId].batas_create_akun = batas_create_akun;
        await ctx.reply('ðŸ’° *Silahkan masukkan harga server:*', { parse_mode: 'Markdown' });
        return;
    } else if (state.step === 'addserver_harga') {
        const harga = parseFloat(ctx.message.text.trim());
        if (isNaN(harga) || harga <= 0) {
            return ctx.reply('âš ï¸ *Harga tidak valid.* Silahkan masukkan harga server yang valid.', { parse_mode: 'Markdown' });
        }
    ...
    if (state.step === 'add_saldo') {
        ...
        if (isNaN(amount) || amount <= 0) {
            return ctx.reply('âš ï¸ *Jumlah saldo tidak valid. Masukkan angka positif.*', { parse_mode: 'Markdown' });
        }
        ...
        try {
            ...
            if (changes > 0) {
                ctx.reply(`âœ… *Saldo sebesar \`${amount}\` berhasil ditambahkan ke user ID \`${targetUserId}\`.*`, { parse_mode: 'Markdown' });
            } else {
                ctx.reply('âš ï¸ *Pengguna tidak ditemukan atau saldo tidak berubah.*', { parse_mode: 'Markdown' });
            }
        } catch (err) {
            logger.error('âŒ Terjadi kesalahan saat menambahkan saldo user:', err.message);
            ctx.reply('âŒ *Terjadi kesalahan saat menambahkan saldo user.*', { parse_mode: 'Markdown' });
        }
        delete userState[userId];
        return;
    }
    ...
    if (state && editSteps.includes(state.step)) {
        ...
        if (isNumeric && (isNaN(parseInt(newValue, 10)) || parseInt(newValue, 10) < 0)) {
            return ctx.reply(`âš ï¸ *${fieldName} tidak valid.* Masukkan angka positif yang valid.`, { parse_mode: 'Markdown' });
        }
        if (!newValue) {
            return ctx.reply(`âš ï¸ *${fieldName} tidak boleh kosong.*`, { parse_mode: 'Markdown' });
        }
        ...
        try {
            ...
            const changes = await new Promise((resolve, reject) => {
                db.run(query, [valueToStore, state.serverId], function(err) {
                    if (err) {
                        logger.error(`âš ï¸ Kesalahan saat mengedit ${fieldName} server:`, err.message);
                        reject(err);
                    } else {
                        resolve(this.changes);
                    }
            ...
            if (changes > 0) {
                ctx.reply(`âœ… *${fieldName} server berhasil diubah menjadi \`${newValue}\`.*`, { parse_mode: 'Markdown' });
            } else {
                ctx.reply(`âš ï¸ *Server tidak ditemukan atau ${fieldName} tidak berubah.*`, { parse_mode: 'Markdown' });
            }
        } catch (error) {
            logger.error(`âŒ Error saat mengedit ${fieldName} server:`, error.message);
            ctx.reply(`âŒ *Terjadi kesalahan saat mengedit ${fieldName} server.*`, { parse_mode: 'Markdown' });
        }
        delete userState[userId];
        return;
    }
    ...
    if (state.step === 'edit_harga') {
        ...
        if (isNaN(hargaBaru) || hargaBaru <= 0) {
            return ctx.reply('âš ï¸ *Harga tidak valid. Masukkan angka positif yang valid.*', { parse_mode: 'Markdown' });
        }
        ...
        try {
            ...
            if (changes > 0) {
                ctx.reply(`âœ… *Harga server berhasil diubah menjadi \`Rp${hargaBaru}\`.*`, { parse_mode: 'Markdown' });
            } else {
                ctx.reply('âš ï¸ *Server tidak ditemukan atau harga tidak berubah.*', { parse_mode: 'Markdown' });
            }
        } catch (error) {
            logger.error('âŒ Error saat mengedit harga server:', error.message);
            ctx.reply('âŒ *Terjadi kesalahan saat mengedit harga server.*', { parse_mode: 'Markdown' });
        }
        delete userState[userId];
        return;
    }
...
bot.action('addserver', async (ctx) => {
  try {
    ...
    await ctx.answerCbQuery();
    await ctx.reply('ðŸŒ *Silahkan masukkan domain/ip server:*', { parse_mode: 'Markdown' });
...
bot.action('detailserver', async (ctx) => {
  try {
    ...
    await ctx.reply('ðŸ“‹ *Silahkan pilih server untuk melihat detail:*', {
...
bot.action('deleteserver', async (ctx) => {
  try {
    logger.info('ðŸ—‘ï¸ Proses hapus server dimulai');
    await ctx.answerCbQuery();
    ...
    db.all('SELECT * FROM Server', [], (err, servers) => {
      ...
      keyboard.push([{ text: 'ðŸ”™ Kembali ke Menu Utama', callback_data: 'kembali_ke_menu' }]);

      ctx.reply('ðŸ—‘ï¸ *Pilih server yang ingin dihapus:*', {
        reply_markup: {
          inline_keyboard: keyboard
        },
        parse_mode: 'Markdown'
    ...
    logger.error('âŒ Kesalahan saat memulai proses hapus server:', error);
    await ctx.reply('âŒ *GAGAL! Terjadi kesalahan saat memproses permintaan Anda. Silahkan coba lagi nanti.*', { parse_mode: 'Markdown' });
  }
});
...
bot.action('cek_saldo', async (ctx) => {
  try {
    ...
    const row = await new Promise((resolve, reject) => {
      db.get('SELECT saldo FROM users WHERE user_id = ?', [userId], (err, row) => {
        if (err) {
          logger.error('âŒ Kesalahan saat memeriksa saldo:', err.message);
          return reject('âŒ *Terjadi kesalahan saat memeriksa saldo Anda. Silahkan coba lagi nanti.*');
        }
        resolve(row);
      });
    });
    ...
    if (row) {
      await ctx.reply(`ðŸ“Š *Cek Saldo*\n\nðŸ†” ID Telegram: ${userId}\nðŸ’° Sisa Saldo: Rp${row.saldo}`, 
      { 
        parse_mode: 'Markdown', 
        reply_markup: {
          inline_keyboard: [
            [{ text: 'ðŸ’¸ Top Up', callback_data: 'menu_topup' }, { text: 'ðŸ“ Menu Utama', callback_data: 'send_main_menu' }]
          ]
        } 
      });
    } else {
      await ctx.reply('âš ï¸ *Anda belum memiliki saldo. Silahkan tambahkan saldo terlebih dahulu.*', { parse_mode: 'Markdown' });
    }
    
  } catch (error) {
    logger.error('âŒ Kesalahan saat memeriksa saldo:', error);
    await ctx.reply(`âŒ *${error.message}*`, { parse_mode: 'Markdown' });
  }
});
...
const getUsernameById = async (userId) => {
  try {
    ...
    logger.error('âŒ Kesalahan saat mengambil username dari Telegram:', err.message);
  ...
  try {
    ...
    const users = await new Promise((resolve, reject) => {
      db.all('SELECT id, user_id FROM Users LIMIT 20', [], (err, users) => {
        if (err) {
          logger.error('âŒ Kesalahan saat mengambil daftar user:', err.message);
    ...
    const totalUsers = await new Promise((resolve, reject) => {
      db.get('SELECT COUNT(*) as count FROM Users', [], (err, row) => {
        if (err) {
          logger.error('âŒ Kesalahan saat menghitung total user:', err.message);
    ...
    await ctx.reply('ðŸ“Š *Silahkan pilih user untuk menambahkan saldo:*', {
      reply_markup: replyMarkup,
      parse_mode: 'Markdown'
    });
  } catch (error) {
    logger.error('âŒ Kesalahan saat memulai proses tambah saldo user:', error);
    await ctx.reply(`âŒ *${error}*`, { parse_mode: 'Markdown' });
  }
});
bot.action(/next_users_(\d+)/, async (ctx) => {
  ...
  try {
    ...
    logger.error('âŒ Kesalahan saat memproses next users:', error);
    await ctx.reply(`âŒ *${error}*`, { parse_mode: 'Markdown' });
  }
});
...
bot.action(/prev_users_(\d+)/, async (ctx) => {
  ...
  try {
    ...
    logger.error('âŒ Kesalahan saat memproses previous users:', error);
    await ctx.reply(`âŒ *${error}*`, { parse_mode: 'Markdown' });
  }
});
  ...
  try {
    ...
    const servers = await new Promise((resolve, reject) => {
      db.all('SELECT id, nama_server FROM Server', [], (err, servers) => {
        if (err) {
          logger.error('âŒ Kesalahan saat mengambil daftar server:', err.message);
          return reject('âš ï¸ *PERHATIAN! Terjadi kesalahan saat mengambil daftar server.*');
        }
        resolve(servers);
      });
    ...
    await ctx.reply('ðŸ“Š *Silahkan pilih server untuk mengedit limit IP:*', {
      reply_markup: { inline_keyboard: inlineKeyboard },
      parse_mode: 'Markdown'
    });
  } catch (error) {
    logger.error('âŒ Kesalahan saat memulai proses edit limit IP server:', error);
    await ctx.reply(`âŒ *${error}*`, { parse_mode: 'Markdown' });
  }
});
bot.action('editserver_batas_create_akun', async (ctx) => {
  try {
    ...
    await ctx.reply('ðŸ“Š *Silahkan pilih server untuk mengedit batas create akun:*', {
      reply_markup: { inline_keyboard: inlineKeyboard },
      parse_mode: 'Markdown'
    });
  } catch (error) {
    logger.error('âŒ Kesalahan saat memulai proses edit batas create akun server:', error);
    await ctx.reply(`âŒ *${error}*`, { parse_mode: 'Markdown' });
  }
});
bot.action('editserver_total_create_akun', async (ctx) => {
  try {
    ...
    await ctx.reply('ðŸ“Š *Silahkan pilih server untuk mengedit total create akun:*', {
      reply_markup: { inline_keyboard: inlineKeyboard },
      parse_mode: 'Markdown'
    });
  } catch (error) {
    logger.error('âŒ Kesalahan saat memulai proses edit total create akun server:', error);
    await ctx.reply(`âŒ *${error}*`, { parse_mode: 'Markdown' });
  }
});
bot.action('editserver_quota', async (ctx) => {
  try {
    ...
    await ctx.reply('ðŸ“Š *Silahkan pilih server untuk mengedit quota:*', {
      reply_markup: { inline_keyboard: inlineKeyboard },
      parse_mode: 'Markdown'
    });
  } catch (error) {
    logger.error('âŒ Kesalahan saat memulai proses edit quota server:', error);
    await ctx.reply(`âŒ *${error}*`, { parse_mode: 'Markdown' });
  }
});
bot.action('editserver_auth', async (ctx) => {
  try {
    ...
    await ctx.reply('ðŸŒ *Silahkan pilih server untuk mengedit auth:*', {
      reply_markup: { inline_keyboard: inlineKeyboard },
      parse_mode: 'Markdown'
    });
  } catch (error) {
    logger.error('âŒ Kesalahan saat memulai proses edit auth server:', error);
    await ctx.reply(`âŒ *${error}*`, { parse_mode: 'Markdown' });
  }
});
...
bot.action('editserver_harga', async (ctx) => {
  try {
    ...
    await ctx.reply('ðŸ’° *Silahkan pilih server untuk mengedit harga:*', {
      reply_markup: { inline_keyboard: inlineKeyboard },
      parse_mode: 'Markdown'
    });
  } catch (error) {
    logger.error('âŒ Kesalahan saat memulai proses edit harga server:', error);
    await ctx.reply(`âŒ *${error}*`, { parse_mode: 'Markdown' });
  }
});
...
bot.action('editserver_domain', async (ctx) => {
  try {
    ...
    await ctx.reply('ðŸŒ *Silahkan pilih server untuk mengedit domain:*', {
      reply_markup: { inline_keyboard: inlineKeyboard },
      parse_mode: 'Markdown'
    });
  } catch (error) {
    logger.error('âŒ Kesalahan saat memulai proses edit domain server:', error);
    await ctx.reply(`âŒ *${error}*`, { parse_mode: 'Markdown' });
  }
});
...
bot.action('nama_server_edit', async (ctx) => {
  try {
    ...
    await ctx.reply('ðŸ·ï¸ *Silahkan pilih server untuk mengedit nama:*', {
      reply_markup: { inline_keyboard: inlineKeyboard },
      parse_mode: 'Markdown'
    });
  } catch (error) {
    logger.error('âŒ Kesalahan saat memulai proses edit nama server:', error);
    await ctx.reply(`âŒ *${error}*`, { parse_mode: 'Markdown' });
  }
});
...
bot.action('topup_saldo', async (ctx) => {
  ...
  try {
    await ctx.answerCbQuery(); 
    logger.info(`ðŸ” User ${userId} memulai proses top-up saldo.`);

    if (lastMenus[userId]) {
      try {
        await bot.telegram.deleteMessage(chatId, lastMenus[userId]);
        logger.info(`ðŸ§¹ Menu lama milik ${userId} berhasil dihapus`);
        delete lastMenus[userId];
      } catch (e) {
        console.warn(`âš ï¸ Gagal menghapus menu sebelumnya untuk ${userId}:`, e.message);
      }
    }
    ...
    logger.error('âŒ Kesalahan saat memulai proses top-up saldo:', error);
    try {
      await ctx.reply(
        'âŒ *GAGAL! Terjadi kesalahan saat memproses permintaan Anda. Silahkan coba lagi nanti.*',
        { parse_mode: 'Markdown' }
      );
    } catch (e) {
...
bot.action('topup_saweria', async (ctx) => {
  ...
  try {
    ...
    // âœ… Simpan state bahwa user diminta masukkan nominal
    if (!global.depositState) global.depositState = {};
    ...
    logger.info(`ðŸ“ Menunggu input nominal dari user ${userId}`);

    // Kirim instruksi ke user
    const sent = await ctx.reply(
      'ðŸ’° *Silahkan ketik nominal top-up yang ingin Anda bayarkan melalui QRIS Saweria.*\n\nContoh: `1000`',
      { parse_mode: 'Markdown' }
    );
    ...
    logger.error('âŒ Kesalahan saat memulai proses top-up Saweria:', error);
    try {
      await ctx.reply(
        'âŒ *GAGAL!* Terjadi kesalahan saat memproses top-up Saweria Anda. Silakan coba lagi nanti.',
        { parse_mode: 'Markdown' }
      );
    } catch (e) {
      logger.error('Gagal kirim pesan error:', e.message);
...
bot.action(/^saweria_nominal_(\d+)$/, async (ctx) => {
  ...
  await ctx.answerCbQuery();
  await ctx.reply(`ðŸ”„ Memproses QRIS Saweria untuk Rp${amount}...`);
...
bot.action('bonus_topup_setting', async (ctx) => {
    ...
    db.get('SELECT * FROM bonus_config WHERE id = 1', (err, row) => {
        if (err || !row) {
            return ctx.reply('âŒ Gagal mengambil pengaturan bonus.');
        }

        ctx.reply(`âš™ï¸ *Pengaturan Bonus Top Up*

` +
            `Status: *${row.enabled ? 'Aktif âœ…' : 'Nonaktif âŒ'}*
` +
...
` +
            ...
            reply_markup: {
                inline_keyboard: [
                    [{ text: 'ðŸ”§ Atur Bonus TopUp', callback_data: 'atur_bonus_topup' }]
                ]
            }
        });
    });
...
bot.action('atur_bonus_topup', async (ctx) => {
    ...
    await ctx.reply('âœï¸ Kirim format:\n`on|off <minimal_topup> <persen_bonus>`\n\nContoh:\n`on 10000 25`', {
        parse_mode: 'Markdown'
    });
});
...
bot.action('log_bonus_topup', async (ctx) => {
  ...
  db.all('SELECT * FROM bonus_log ORDER BY id DESC LIMIT 10', [], (err, rows) => {
    if (err || rows.length === 0) {
      return ctx.reply('âš ï¸ Belum ada data bonus');
    }
    ...
    const isi = rows.map((row, i) => {
      ...
      return `*${i + 1}.* ${username}\nðŸ†” ID: \`${row.user_id}\`\nðŸ’¸ TopUp: Rp${row.amount}\nðŸŽ Bonus: Rp${row.bonus}\nðŸ•’ ${formattedTimestamp}`;
    }).join('\n\n');

    ctx.reply(`ðŸ“‹ *Riwayat Bonus Top Up (10 Terbaru)*\n\n${isi}`, {
      parse_mode: 'Markdown'
    });
  });
});
...
bot.action('log_topup', async (ctx) => {
  ...
  db.all('SELECT * FROM topup_log ORDER BY id DESC LIMIT 10', [], (err, rows) => {
    if (err || rows.length === 0) {
      return ctx.reply('âš ï¸ Belum ada data topup');
    }
    ...
    ctx.reply(`ðŸ“‹ *Riwayat Top Up (10 Terbaru)*\n\n${isi}`, {
      parse_mode: 'Markdown'
    });
  });
});
...
function prosesBonusTopUp(user_id, username, original_amount) {
    db.get('SELECT * FROM bonus_config WHERE id = 1', (err, config) => {
        ...
        if (config.enabled && original_amount >= config.min_topup) {
            ...
            bot.telegram.sendMessage(user_id, `ðŸŽ *Bonus Top Up!* Kamu dapat saldo tambahan *Rp${bonus}* (${config.bonus_percent}%)`, {
                parse_mode: 'Markdown'
            });
        }
    });
...
function logTopup(user_id, username, amount, method) {
  db.run(
    ...
    (err) => {
      if (err) {
        logger.error('âŒ Gagal insert ke topup_log:', err.message);
      } else {
        logger.info(`âœ… Log Topup: ${user_id} - ${username} - Rp${amount} - ${method}`);
      }
    }
  );
}
...
bot.action(/edit_harga_(\d+)/, async (ctx) => {
  ...
  await ctx.reply('ðŸ’° *Silahkan masukkan harga server baru:*', {
...
bot.action(/add_saldo_(\d+)/, async (ctx) => {
  ...
  await ctx.reply('ðŸ“Š *Silahkan masukkan jumlah saldo yang ingin ditambahkan:*', {
    reply_markup: { inline_keyboard: keyboard_nomor() },
    parse_mode: 'Markdown'
  });
});
bot.action(/edit_batas_create_akun_(\d+)/, async (ctx) => {
  ...
  await ctx.reply('ðŸ“Š *Silahkan masukkan batas create akun server baru:*', {
    reply_markup: { inline_keyboard: keyboard_nomor() },
    parse_mode: 'Markdown'
  });
});
bot.action(/edit_total_create_akun_(\d+)/, async (ctx) => {
  ...
  await ctx.reply('ðŸ“Š *Silahkan masukkan total create akun server baru:*', {
    reply_markup: { inline_keyboard: keyboard_nomor() },
    parse_mode: 'Markdown'
  });
});
bot.action(/edit_limit_ip_(\d+)/, async (ctx) => {
  ...
  await ctx.reply('ðŸ“Š *Silahkan masukkan limit IP server baru:*', {
    reply_markup: { inline_keyboard: keyboard_nomor() },
    parse_mode: 'Markdown'
  });
});
bot.action(/edit_quota_(\d+)/, async (ctx) => {
  ...
  await ctx.reply('ðŸ“Š *Silahkan masukkan quota server baru:*', {
    reply_markup: { inline_keyboard: keyboard_nomor() },
    parse_mode: 'Markdown'
  });
});
bot.action(/edit_auth_(\d+)/, async (ctx) => {
  ...
  await ctx.reply('âœï¸ *Silahkan kirim auth server baru sekarang:*', {
    parse_mode: 'Markdown'
  });
});
bot.action(/edit_domain_(\d+)/, async (ctx) => {
  ...
  await ctx.reply('ðŸŒ *Silahkan kirim domain server baru sekarang:*', {
    parse_mode: 'Markdown'
  });
});
bot.action(/edit_nama_(\d+)/, async (ctx) => {
  ...
  await ctx.reply('ðŸ·ï¸ *Silahkan kirim nama server baru sekarang:*', {
    parse_mode: 'Markdown'
  });
});
...
async function handleDepositState(ctx, userId, data) {
  ...
  if (data === 'delete') {
    ...
    if (parseInt(currentAmount) < 100) {
      return await ctx.answerCbQuery('âš ï¸ Jumlah minimal top-up adalah 100 Ya Kak...!!!', { show_alert: true });
    }
    ...
    try {
      ...
      logger.warn(`âš ï¸ Gagal menghapus pesan top-up konfirmasi: ${e.message}`);
    }
    ...
    if (currentAmount.length < maxDigits) {
      ...
      return await ctx.answerCbQuery(`âš ï¸ Jumlah maksimal adalah ${maxDigits} digit!`, { show_alert: true });
    }
  }
  ...
  const newMessage =
    action === 'request_amount_saweria'
      ? `ðŸ’° Masukkan nominal topup Saweria QRIS:\n\nNominal saat ini: *Rp${currentAmount}*`
      : `ðŸ’³ Topup Saldo Otomatis QRIS\nâ”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”\nMasukkan nominal topup:\n\nRp ${currentAmount}\n\nMinimal topup Rp 100\nâ”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”\nGunakan tombol di bawah untuk input nominal.`;
  ...
  try {
    ...
    logger.error('âŒ Gagal update pesan nominal top-up:', error);
  }
}
...
async function handleAddSaldo(ctx, userStateData, data) {
  ...
  const newMessage = `ðŸ“Š *Silahkan masukkan jumlah saldo yang ingin ditambahkan:*\n\nJumlah saldo saat ini: *${currentSaldo}*`;
...
async function handleEditHarga(ctx, userStateData, data) {
  ...
  const newMessage = `ðŸ’° *Silahkan masukkan harga server baru:*\n\nJumlah saat ini: *Rp ${currentAmount}*`;
  if (newMessage !== ctx.callbackQuery.message.text) {
    await ctx.editMessageText(newMessage, {
      reply_markup: { inline_keyboard: keyboard_nomor() },
      parse_mode: 'Markdown'
...
async function handleEditField(ctx, userStateData, data, field, fieldName, query) {
  ...
  const newMessage = `ðŸ“Š *Silahkan masukkan ${fieldName} server baru:*\n\n${fieldName.charAt(0).toUpperCase() + fieldName.slice(1)} saat ini: *${currentValue}*`;
  if (newMessage !== ctx.callbackQuery.message.text) {
    await ctx.editMessageText(newMessage, {
      reply_markup: { inline_keyboard: keyboard_nomor() },
      parse_mode: 'Markdown'
...
const config = {
    storeName: NAMA_STORE, // Nama toko
    auth_username: MERCHANT_ID, // Username OrderKuota
    auth_token: API_KEY, // Token OrderKuota
    baseQrString: DATA_QRIS, // String QRIS statis
    logoPath: 'logo.png' // Opsional, path logo
};
...
async function processDeposit(ctx, amount) {
  ...
  if (currentTime - lastRequestTime < requestInterval) {
    await ctx.reply('âš ï¸ *Terlalu banyak permintaan. Silahkan tunggu sebentar sebelum mencoba lagi.*', { parse_mode: 'Markdown' });
    return;
  }
...
let waitMsg = await ctx.reply("â³ Mohon menunggu.");
...
const interval = setInterval(async () => {
  i = (i + 1) % dots.length;
  try {
    await ctx.telegram.editMessageText(ctx.chat.id, waitMsg.message_id, null, `â³ Mohon menunggu${dots[i]}`);
  } catch (e) {
    clearInterval(interval);
  }
}, 1000);
...
delete global.depositState[userId];
    ...
    logger.error('âŒ Kesalahan saat memproses deposit:', error);
    await ctx.reply('âŒ *GAGAL! Terjadi kesalahan saat memproses pembayaran. Silahkan coba lagi nanti.*', { parse_mode: 'Markdown' });
...
async function sendPaymentSuccessNotificationGrup(userId, deposit, ctx = null) {
  return new Promise(async (resolve) => {
    try {
      ...
      const userDisplay = userInfo.username
        ? `@${userInfo.username}`
        : (userInfo.first_name || `ID ${userId}`);

      // Ambil saldo user dari database
      db.get('SELECT saldo FROM users WHERE user_id = ?', [userId], async (err, user) => {
        if (err || !user) {
          logger.error('Gagal ambil data user untuk notifikasi topup:', err?.message || 'User tidak ditemukan');
          return resolve(false);
        }

        const message = `<blockquote>
â—‡â”â”â”â”â”â”â”â”â”â”â”â”â”â”â—‡
 <b>  âš ï¸NOTIFICATIONS TOPUPâš ï¸</b>
     Detail Transaxsi Topup
â—‡â”â”â”â”â”â”â”â”â”â”â”â”â”â”â—‡
âœ… <b>Top Up Berhasil</b>
ðŸ‘¤ User: ${userDisplay}xxx
ðŸ’° Nominal: <b>Rp ${deposit.originalAmount}</b>
ðŸ¦ Saldo Sekarang: <b>Rp ${user.saldo}</b>
ðŸ•’ Waktu: ${new Date().toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })}
â—‡â”â”â”â”â”â”â”â”â”â”â”â”â”â”â—‡
ðŸ Otomatis Notification Order @regisx_bot
</blockquote>`;

        // Kirim ke user (opsional)
//        try {
//          await bot.telegram.sendMessage(userId, message, { parse_mode: 'HTML' });
//        } catch (e) {
//          logger.warn(`Gagal kirim ke user ${userId}:`, e.message);
//        }
        ...
        try {
          if (GROUP_CHAT_ID) {
            ...
            logger.warn('GROUP_CHAT_ID tidak diatur di .vars.json');
          }
        } catch (e) {
          logger.error('Gagal kirim notifikasi ke grup:', e.message);
        }
      ...
      logger.error('âŒ Error sendPaymentSuccessNotification:', e.message);
      resolve(false);
    }
  });
}
...
function keyboard_abc() {
  ...
  buttons.push([{ text: 'ðŸ”™ Kembali ke Menu Utama', callback_data: 'send_main_menu' }]);
  return buttons;
}
...
async function sendPaymentSuccessNotification(userId, deposit, currentBalance) {
  try {
    ...
    const bonusText = hasBonus
      ? `ðŸš€ Bonus Top Up: *Rp${deposit.bonus}* (${deposit.bonus_percent}%)\n`
      : '';
    const adminFee = deposit.amount - deposit.originalAmount;
    const messageText = 
      `âœ… *Pembayaran Berhasil!*\n\n` +
      `ðŸ’° Jumlah Deposit: Rp ${deposit.originalAmount}\n` +
      `ðŸ’° Biaya Admin: Rp ${adminFee}\n` +
      `ðŸ’³ Saldo ditambahkan: Rp ${deposit.originalAmount}\n` +
      bonusText +
      `ðŸ¦ Saldo sekarang: Rp ${saldo}`;
    ...
    await bot.telegram.sendMessage(userId, messageText, {
      ...
      reply_markup: {
        inline_keyboard: [
          [
            { text: 'ðŸ’¸ Top Up', callback_data: 'menu_topup' },
    ...
    // === TAMBAHKAN: HAPUS QRIS PAYMENT MESSAGE JIKA ADA ===
    if (deposit.qrMessageId) {
      try {
    ...
    logger.error('âŒ Error sending payment notification:', error);
    return false;
  }
}
...
async function processMatchingPayment(deposit, matchingTransaction, uniqueCode) {
  ...
  try {
    ...
    deposit.username = telegramUser.username || '';
  } catch (e) {
    deposit.username = '';
  }
}
    ...
    try {
        ...
        logTopup(deposit.userId, deposit.username, deposit.originalAmount, 'QRIS Orkut');
        await sendPaymentSuccessNotificationGrup(deposit.userId, {
  originalAmount: deposit.original_amount || deposit.amount || 0
});
...
async function kirimFileKeTelegram() {
  ...
  if (!fs.existsSync(FOLDER_TEMPATDB)) {
    console.log("âŒ File tidak ditemukan:", FOLDER_TEMPATDB);
    return;
  }
  ...
  try {
    ...
    if (data.ok) {
      ...
      console.error("âŒ Gagal mengirim file:", data.description);
    }
  } catch (err) {
    console.error("âŒ Error saat mengirim file:", err.message);
  }
}
...
app.listen(port, () => {
  ...
  logger.info(`Server berjalan di port ${port}`);
    ...
    bot.sendMessage(chatId, 'âŒ Terjadi kesalahan saat memproses perintah.');
  }
});

// ðŸ”¥ Global error catcher
process.on('uncaughtException', async (err) => {
  console.error('âŒ BOT CRASH:', err);
  let msg = `ðŸš¨ *BOT CRASH*\n*Time:* ${new Date().toLocaleString()}\n*Error:* \`\`\`${err.message}\`\`\``;
  if (global.lastUserId) {
    msg += `\n*User:* ${global.lastUserId}`;
    addToBlacklist(global.lastUserId);
  }
  try {
    await bot.sendMessage(ADMIN_ID, msg, { parse_mode: 'Markdown' });
  } catch (e) {
    console.error('Gagal kirim laporan ke admin:', e.message);
  }
  process.exit(1);
});