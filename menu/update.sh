...
username=$(curl -sS curl -sS https://raw.githubusercontent.com/GeKaStore/license/main/ip | grep $MYIP | awk '{print $2}')
valid=$(curl -sS curl -sS https://raw.githubusercontent.com/GeKaStore/license/main/ip | grep $MYIP | awk '{print $3}')
today=$(date +"%Y-%m-%d")
d1=$(date -d "$valid" +%s)
d2=$(date -d "$today" +%s)
