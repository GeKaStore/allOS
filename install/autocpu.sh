...
data_ip="curl -sS https://raw.githubusercontent.com/GeKaStore/license/main/ip"
checking_sc() {
    ...
    if [[ $(date -d "$date_list" +%s) -lt $(date -d "$useexp" +%s) ]]; then
        ...
        REPO="https://raw.githubusercontent.com/GeKaStore/allOS/main/" # Ganti dengan URL repository Anda
        ...
        if [[ $serverV == $localV ]]; then
            ...
            wget -q https://raw.githubusercontent.com/GeKaStore/allOS/main/menu/update.sh -O update.sh
            chmod +x update.sh
            ./update.sh
